...
with 
q1 as (
SELECT *  FROM {{source("tourism", "unwto_outbound_tourism_departures")}}
WHERE COALESCE(
    country, type_of_travel, units, 
...
select * from q1 
where coalesce(country, type_of_travel) is not null 
), 
q3 as (
...
select 
    ...
    CAST(type_of_travel as VARCHAR) as type_of_travel,
